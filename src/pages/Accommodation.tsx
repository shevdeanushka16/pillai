...
export const Accommodation: React.FC = () => {
  const { hotels, simulationState, toggleAction, actions, addToast, currentEvent } = useApp();

  const shiftAction = actions.find((a) => a.id === 'shift-stays');
  const isShifted = shiftAction?.applied || simulationState === 'optimized';

  const totalInventory = hotels.reduce((acc, h) => acc + h.totalRooms, 0);
  const totalAvailable = hotels.reduce((acc, h) => acc + h.availableRooms, 0);
  const zoneCHotels = hotels.find((h) => h.zone === 'Zone C');
  const zoneCAvailable = zoneCHotels ? zoneCHotels.availableRooms : 940;
  const rebalanceTargetRooms = Math.min(zoneCAvailable, Math.round(currentEvent.expectedAttendance * 0.012));
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <span className="text-[11px] font-mono tracking-wider uppercase text-secondary block">
            Hospitality & Lodging • {currentEvent.city}
          </span>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-primary font-mono">
            ACCOMMODATION CAPACITY
          </h1>
          <p className="text-sm text-secondary mt-1">
            Real-time hotel inventory and room availability across {currentEvent.title} districts
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {isShifted ? (
              ...
              <>
                <span>REBALANCE TO ZONE C ({rebalanceTargetRooms.toLocaleString()} ROOMS)</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-surface border border-border p-4 rounded-md">
          ...
          <span className="text-2xl font-bold font-mono text-primary">
            {hotels.reduce((acc, h) => acc + h.hotelsCount, 0)}
          </span>
        ...
        <div className="bg-surface border border-border p-4 rounded-md">
          ...
          <span className="text-[11px] text-secondary block mt-0.5">Out of {totalInventory.toLocaleString()} inventory</span>
        </div>
        <div className="bg-surface border border-border p-4 rounded-md">
          <span className="text-[10px] font-mono uppercase text-secondary block">OVERALL OCCUPANCY</span>
          <span className="text-2xl font-bold font-mono text-primary">
            {Math.round(
              ((totalInventory - totalAvailable) / (totalInventory || 1)) * 100
            )}%
          </span>
          <span className="text-[11px] text-secondary block mt-0.5">{currentEvent.venueName} perimeter</span>
        </div>
        <div className="bg-surface border border-border p-4 rounded-md">
          ...
          <span className="text-[11px] text-secondary block mt-0.5">{zoneCAvailable.toLocaleString()} rooms available</span>
        </div>
      </div>
