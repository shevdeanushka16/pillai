...
export const CrowdAnalytics: React.FC = () => {
  const { zones, simulationState, currentEvent } = useApp();
  ...
  // Base scale ratio compared to default 82,500 attendance
  const scale = currentEvent ? currentEvent.expectedAttendance / 82500 : 1;

  const hourlyTrendData = [
    { time: '2:00 PM', zoneA: 20, zoneB: 15, zoneC: 18, total: Math.round(14000 * scale) },
    { time: '3:00 PM', zoneA: 38, zoneB: 28, zoneC: 32, total: Math.round(28000 * scale) },
    { time: '4:00 PM', zoneA: 55, zoneB: 42, zoneC: 45, total: Math.round(42000 * scale) },
    { time: '5:00 PM', zoneA: 72, zoneB: 50, zoneC: 60, total: Math.round(61000 * scale) },
    { time: '6:00 PM', zoneA: isSurge ? 86 : isOptimized ? 74 : 64, zoneB: 56, zoneC: isSurge ? 78 : 50, total: Math.round((isSurge ? 82000 : 68000) * scale) },
    { time: '7:00 PM', zoneA: isSurge ? 96 : isOptimized ? 72 : 70, zoneB: 62, zoneC: isSurge ? 88 : 54, total: Math.round((isSurge ? 96000 : 72000) * scale) },
    { time: '8:00 PM', zoneA: isSurge ? 92 : isOptimized ? 68 : 66, zoneB: 58, zoneC: isSurge ? 76 : 48, total: Math.round((isSurge ? 89000 : 67000) * scale) },
    { time: '9:00 PM', zoneA: 70, zoneB: 52, zoneC: 65, total: Math.round(60000 * scale) },
    { time: '10:00 PM', zoneA: 35, zoneB: 30, zoneC: 40, total: Math.round(32000 * scale) },
  ];

  const peakAttendees = currentEvent 
    ? Math.round(currentEvent.expectedAttendance * (isSurge ? 1.08 : 1.02)) 
    : 96000;
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <span className="text-[11px] font-mono tracking-wider uppercase text-secondary block">
            Spatial Distribution • {currentEvent.venueName}
          </span>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-primary font-mono">
            CROWD ANALYTICS
          </h1>
          <p className="text-sm text-secondary mt-1">
            Real-time zone density metrics and turnstile queue telemetry for {currentEvent.title}
          </p>
        </div>
        ...
        <div className="bg-surface border border-border rounded-md px-5 py-3 flex items-center gap-5">
          <div>
            ...
            <div className="text-xl font-bold font-mono text-primary">
              {peakAttendees.toLocaleString()} visitors
            </div>
            <span className="text-xs font-mono text-high font-semibold">
              7:00 PM ({currentEvent.type === 'sports' ? 'Final Innings' : 'Headliner'})
            </span>
          </div>
