...
export const SitrepModal: React.FC = () => {
  const { showSitrepModal, setShowSitrepModal, metrics, pressureScore, riskEvaluation, simulationState, simulatedTime, actions, currentEvent } = useApp();
...
EVENT: ${currentEvent.title} (${currentEvent.venueName})
LOCATION: ${currentEvent.city} | DATE: ${currentEvent.date}
TIME: ${simulatedTime} IST
STATUS: ${riskEvaluation.level} (Pressure Score: ${pressureScore}/100)
=====================================================

1. TELEMETRY SUMMARY:
- Inbound Attendees: ${metrics.visitors.toLocaleString()} pax (Max: ${currentEvent.capacity.toLocaleString()})
...
${
  pressureScore >= 80
    ? `- CRITICAL STRAIN: ${currentEvent.primaryGate} turnstiles experiencing severe queues (>28 min wait). ${currentEvent.highwayCorridor} at 92% vehicular congestion.`
    : pressureScore >= 60
    ? `- HIGH STRAIN: Increasing arrivals ahead of peak period. ${currentEvent.primaryGate} approaching saturation buffer.`
...
${actions
  .filter((a) => a.applied)
  .map((a) => `[ACTIVE] ${a.number} ${a.title} - ${a.id === 'divert-arrivals' ? `${currentEvent.primaryGate} → ${currentEvent.alternateGate}` : a.id === 'activate-shuttle' ? currentEvent.shuttleRoute : a.subtitle} (${a.expectedImpact})`)
  .join('\n') || '- None currently deployed (Standby mode).'}

4. FIELD DISPATCH DIRECTIVES:
- Transit Police: Enforce bus priority lane on ${currentEvent.shuttleRoute} (${currentEvent.alternateGate} bypass).
- Venue Security: Open all ${currentEvent.gatesCount || 14} scanning lanes at ${currentEvent.alternateGate}; broadcast digital signage rerouting ${currentEvent.primaryGate} queues.
- Hospitality: Trigger automated room incentives for secondary hospitality zones.
